...
def fetch_price_data(companies, client):
    ...
    for i, stock in enumerate(companies):  
        try:
            ...
            print(f"Fetching {stock} ({i+1}/{len(companies)})...")
            ...
            empty_df = pd.DataFrame(columns=[stock])
            prices_data = prices_data.join(empty_df, how="outer") if not prices_data.empty else empty_df
